...
JOptionPane.showMessageDialog(this, "enter customer "+s);
 ...
 {
     try {
           c.stmt=c.con.prepareCall("update customer set CUST_NO=?,NAME=?,ADDRESS=?,COUNTRY=?,CONTACT_NO=?,EMAIL=?,PROOF=?,ID_NO=?,RNO=?,RTYPE=?,PAKAGE=?,ADULT=?,KIDS=?,CIN=?,COUT=?,SECURITY=?,RMSERVICE=? where CUST_NO=?");
